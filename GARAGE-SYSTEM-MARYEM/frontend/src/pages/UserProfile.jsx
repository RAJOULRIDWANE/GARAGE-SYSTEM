...
const UserProfile = () => {
  ...
  const handleSave = async (e) => {
    ...
    if (passwords.newPassword !== passwords.confirmPassword) {
      setMessage({ type: 'error', text: t('profile.mismatch_error') });
      return;
    }
    if (passwords.newPassword.length < 6) {
      setMessage({ type: 'error', text: t('profile.length_error') });
      return;
    }
    ...
    try {
      ...
      if (!token) {
        throw new Error(t('profile.login_error'));
      }
      ...
      if (!response.ok) {
        ...
        const errorMsg = data.message || t('profile.failed_update');
            ...
            <form className="form-content" onSubmit={handleSave}>
              ...
              <div style={{ marginTop: '20px', textAlign: 'right' }}>
                <button type="submit" className="update-btn" disabled={isLoading}>
                  {isLoading ? t('profile.updating') : t('profile.update_button')}
                </button>
              </div>
