...
const ReceptionistDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // SINGLE CALL to get everything: User, Mechanics, Repairs, KPIs, Clients Summary
      const res = await axios.get('http://127.0.0.1:8000/api/receptionist/dashboard', {
        headers: { Authorization: `Bearer ${token}` }
      });

      const { user, mechanics, repairs, clientsSummary } = res.data;
      ...
      setRepairs(repairs || []);
      setGroupedClients(clientsSummary || []);

      // If we want to use the server-calculated KPIs:
      if (res.data.kpis) {
        setServerKPIs(res.data.kpis);
      }
  ...
  // Add a new state for server KPIs to avoid mismatch if repairs are paginated
  const [serverKPIs, setServerKPIs] = useState({ todaysAppointments: 0, confirmedToday: 0 });
  ...
  // --- KPI Logic (Now from server) ---
  const { todaysAppointments, confirmedToday } = serverKPIs;
